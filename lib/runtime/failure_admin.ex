...
defmodule Skitter.Runtime.BackupStore do
  ...
  def masterslave_snapshot(pid, role, context = %Context{_skr: {ref,_}, _epoch: 1}, node_idx, state) do   
    admin_cast(NodeStore.get(:failure_stores, ref, node_idx), {:masterslave, node_idx, {pid, role, RT.node_name_for_context(context), 1, state, context.strategy_dag}})
  end

  def masterslave_snapshot(pid, role, context = %Context{_skr: {ref,_}}, node_idx, state) do   
    admin_cast(NodeStore.get(:failure_stores, ref, node_idx), {:masterslave, node_idx, {pid, role, RT.node_name_for_context(context), context._epoch, state}})
  end

  def sync_snapshot(pid, role, context = %Context{_skr: {ref,_}, _epoch: 1}, node_idx, state) do 
    replica_ids_set = replicate_range(ConstantStore.get(:replica_count, ref), pid, node_idx) |> MapSet.new
  ...
  def sync_snapshot(pid, role, context = %Context{_skr: {ref,_}}, node_idx, state) do 
    replicate_range(ConstantStore.get(:replica_count, ref), pid, node_idx)
      |> Enum.each(&admin_cast(NodeStore.get(:failure_stores, ref, &1), {:snapshot, {pid, role, RT.node_name_for_context(context), context._epoch, state}}))
  end
  ...
  def check_drop_epoch(
    ...
    if undeployed_ops==%{} and Map.get(epoch_recv_count,1)==0 do
      Logger.info("[#{0} - #{epoch_max_count}]")
      FailureObs.notify_epoch_drop(ref, 0)
      {ref, { 1, 
        snapshot_dict, 
        epoch_recv_count 
          |> Map.delete(1) 
  ...
  def check_drop_epoch(
    epoch_tick,
    ref,
    {_, _, epoch_recv_count, _} = data
  ) do 
    if (Map.get(epoch_recv_count,epoch_tick)==0), do: FailureObs.notify_epoch_drop(ref, epoch_tick)
    {ref, data}
  end
  ...
  def update_undeployed_ops({operation_n, deployment}, {0, snapshot_dict, undeployed_ops, epoch_recv_count, epoch_max_count}) do 
    if MapSet.member?(undeployed_ops, operation_n) do
      ...
      {0, snapshot_dict, new_undeployed_ops, new_epoch_recv_count, epoch_max_count}
    else
      new_epoch_recv_count = 
  ...
  def new_snapshot(
    ...
    if MapSet.member?(undeployed_ops, operation_n) do
      ...
      {0, new_snapshot_dict, new_undeployed_ops, new_epoch_recv_count, epoch_max_count + 1}
      ...
      {0, new_snapshot_dict, undeployed_ops, new_epoch_recv_count, epoch_max_count + 1}
    end
  end
  ...
  def new_snapshot(
    ...
    {le, sd, rc, emc} = new_snapshot(input, {0, snapshot_dict, epoch_recv_count, epoch_max_count})
    {le, sd, undeployed_ops, rc, emc}
  end
  ...
  def handle_cast({:notify_epoch_drop, 0}, state), do: {:noreply, state}
  def handle_cast({:notify_epoch_drop, epoch_tick}, {ref, {_lowest_epoch, snapshot_dict, epoch_recv_count, epoch_max_count}}) do
    Logger.info("[#{epoch_tick}]")
    {:noreply, {ref, { epoch_tick, 
  ...
  def handle_cast({:snapshot, {pid, role, operation_n, 1, snapshot_state, deployment}}, {ref, state}) do
  ...
  def handle_cast({:snapshot, {operation_n, 1, deployment}}, {ref, state}) do
    {:noreply, check_drop_epoch(
      1,
      ref,
  ...
  def handle_cast({:snapshot, {pid, role, operation_n, epoch_tick, snapshot_state}}, {ref, state}) do
    {:noreply, check_drop_epoch(
      epoch_tick,
      ref,
  ...
  def handle_cast({:masterslave, node_idx, {pid, role, opn, 1, backup_state, dag}}, {ref, state}) do
    replica_ids_set = replicate_range(ConstantStore.get(:replica_count, ref), pid, node_idx) |> MapSet.new
    NodeStore.get_all(:failure_stores, ref)
      |> Enum.with_index
      |> Enum.each(fn {node, idx} -> 
    ...
    handle_cast({:snapshot, {pid, role, opn, 1, backup_state, dag}}, {ref, state})
  end

  def handle_cast({:masterslave, node_idx, ref, {pid, role, opn, epoch, backup_state}}, {ref, state}) do
    replicate_range(ConstantStore.get(:replica_count, ref), pid, node_idx)
      |> Enum.each(&admin_cast(NodeStore.get(:failure_stores, ref, &1), {:snapshot, {pid, role, opn, epoch, backup_state}}))
    {:noreply, {ref, state}}
  end

  def handle_cast(
    {:fetch_refs, fetcher}, 
    {_, {_, snapshot_dict, _, _}} = data
  ) do
  ...
  def handle_cast(
    {:fetch_backup, backup_ref, role, operation_n, fetcher}, {ref, {lowest_epoch, snapshot_dict, _, epoch_max_count}}
  ) do
    {_, backup_value} = MapMacro.get(snapshot_dict, [operation_n,role,backup_ref]) 
    ...
    new_snapshot_dict = MapMacro.update!(snapshot_dict, [operation_n, role], fn mp_role -> 
      {q, map} = Map.pop!(mp_role, backup_ref)
      Map.put(map, fetcher, :queue.in(q |> drop_while(lowest_epoch-1) |> :queue.get(), :queue.new))
    end)
    
    {:noreply, {ref, {lowest_epoch, new_snapshot_dict, %{}, epoch_max_count}}}
  end
end